...
void setup() {
  ...
  lightUpDown(strip.Color(150, 0, 0));
    // Blue
    lightUpDown(strip.Color(0, 0, 150));
    // Green
    lightUpDown(strip.Color(0, 150, 0));
...
void lightUpDown(uint32_t color) {

  for (int i = 0; i < NUM_LEDS; i++) {
      strip.clear(); // Clear all LEDs
      strip.setPixelColor(i, color);
      strip.show();
      delay(25); // Adjust the delay to control speed
    }
    // Move a single pixel down the strip
    for (int i = NUM_LEDS - 1; i >= 0; i--) {
      strip.clear(); // Clear all LEDs
      strip.setPixelColor(i, color);
      strip.show();
      delay(15); // Adjust the delay to control speed
    }
}
